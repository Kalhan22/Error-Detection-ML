...
for(x in 1:cross_validation){
  ...
  prediction <- ifelse(results >0.44,1,0)
...
for(x in 1:cross_validation){
  ...
  testData <- data_updated[testIndexes, ]
  trainData <- data_updated[-testIndexes, ]
  model <- svm(as.factor(class) ~ seq_c_name_2 + seq_company_name_2 + seq_a_city_2 + seq_state_2  + seq_a_city_3 + seq_state_3 + seq_zip_3,data=trainData,kernel = "linear", C = 10, gamma = 0.000001, class.weights= c("0" = 1, "1" = 1.5))
